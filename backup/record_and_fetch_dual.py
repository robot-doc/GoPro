...
RECORD_SECS    = 40
FINALIZE_SECS  = 2
DOWNLOAD_DIR   = "/home/pi/GoPro_Clips_Backup/GoPro_Clips"
COMBINED_DIR   = "/home/pi/GoPro_Clips_Backup/GoPro_Clips/Combined"
...
def run_connect_script():
    """Run the dual GoPro connection script"""
    script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "connect_dual_gopro.sh"))
    print(f"[INFO] Running dual connection script at: {script_path}")
...
def check_storage_availability():
    ...
    try:
        ...
        if os.path.exists("/home/pi/GoPro_Clips_Backup"):
            # Test write access
            test_file = "/home/pi/GoPro_Clips_Backup/.write_test"
            try:
                with open(test_file, 'w') as f:
                    f.write("test")
                os.remove(test_file)
                print("[INFO] External SSD accessible and writable at /home/pi/GoPro_Clips_Backup")
                
                # Create the full directory structure
                full_download_dir = "/home/pi/GoPro_Clips_Backup/GoPro_Clips" 
                full_combined_dir = "/home/pi/GoPro_Clips_Backup/GoPro_Clips/Combined"
            ...
            print("[WARNING] External SSD path /home/pi/GoPro_Clips_Backup does not exist")
...
def combine_videos(video1_path, video2_path, output_path, timestamp):
    ...
    try:
        ...
        try:
            ...
            print(f"[COMBINE] Combining videos: {os.path.basename(video1_path)} + {os.path.basename(video2_path)}")
            result = subprocess.run(cmd, capture_output=True, text=True)
            ...
            if result.returncode == 0:
                print(f"[COMBINE] Successfully created combined video in {combine_time_sec:.1f} seconds: {output_path}")
                ...
                print("[COMBINE] Retrying with re-encoding...")
                cmd_reencode = [
                    "ffmpeg", "-y",
                    "-f", "concat",
                    "-safe", "0", 
                ...
                reencode_end_time = time.time()
                total_combine_time = reencode_end_time - combine_start_time
                
                if result.returncode == 0:
                    print(f"[COMBINE] Successfully created combined video with re-encoding in {total_combine_time:.1f} seconds: {output_path}")
                    
                    # Create metadata file for combined video
                    combined_size_mb = os.path.getsize(output_path) / (1024*1024)
                    create_combined_metadata_file(output_path, video1_path, video2_path, total_combine_time, combined_size_mb)
                    
                    return True
                else:
                    print(f"[ERROR] ffmpeg re-encoding also failed: {result.stderr}")
                    return False
...
def record_and_fetch_all():
    ...
    if len(downloaded_files) == 2 and 'gopro1' in downloaded_files and 'gopro2' in downloaded_files:
        try:
            ...
            if available_space < (max_file_size * 2):
                ...
                combined_filename = f"{timestamp}_combined_GoPro1+GoPro2.mp4"
                combined_path = os.path.join(combined_dir, combined_filename)
                
                # Queue the combination task for background processing
                queue_video_combination(downloaded_files['gopro1'], downloaded_files['gopro2'], 
...
def main():
    ...
    print(f"[DEBUG] /home/pi/GoPro_Clips_Backup exists: {os.path.exists('/home/pi/GoPro_Clips_Backup')}")
    if os.path.exists('/home/pi/GoPro_Clips_Backup'):
        print(f"[DEBUG] /home/pi/GoPro_Clips_Backup is writable: {os.access('/home/pi/GoPro_Clips_Backup', os.W_OK)}")
        print(f"[DEBUG] Available space on USB: {get_available_space_gb('/home/pi/GoPro_Clips_Backup'):.1f}GB")
    ...
    # Check connections
    if not check_all_gopros_connected():
        try:
            print("[INFO] Not all GoPros connected, running connection script...")
            run_connect_script()
            time.sleep(5)  # Give time for connections to establish
            if not check_all_gopros_connected():
                print("[WARNING] Some GoPros still not connected, continuing anyway...")
        except Exception as e:
    ...
    try:
        while True:
            ...
            if not pcf_input.port[INPUT_PIN]:
                print("\n[TRIGGER] Input is HIGH - starting dual recording")
