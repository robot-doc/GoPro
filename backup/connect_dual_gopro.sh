...
# GoPro 1 Config
GOPRO1_MAC="D0:21:F8:9C:FF:80"
GOPRO1_SSID="HERO8 Black Achim 3"
GOPRO1_PSK="5d3-QNv-MTm"
GOPRO1_IP="10.5.5.9"
WLAN1_INTERFACE="wlan0"

# GoPro 2 Config (you'll need to update these values)
GOPRO2_MAC="C8:52:0D:A5:9A:39"  # Replace with your second GoPro's MAC
GOPRO2_SSID="HERO8 Achim 1"  # Replace with your second GoPro's SSID
GOPRO2_PSK="vDh-p7g-TDj"  # Replace with your second GoPro's password
GOPRO2_IP="10.5.5.9"  # This will be the same IP on its own network
...
(
    connect_gopro "$GOPRO1_MAC" "$GOPRO1_SSID" "$GOPRO1_PSK" "$GOPRO1_IP" "$WLAN1_INTERFACE" "$WPA_CONF1" "GoPro1"
) &
PID1=$!

(
    connect_gopro "$GOPRO2_MAC" "$GOPRO2_SSID" "$GOPRO2_PSK" "$GOPRO2_IP" "$WLAN2_INTERFACE" "$WPA_CONF2" "GoPro2"
) &
PID2=$!
...
if [ $RESULT1 -eq 0 ] && [ $RESULT2 -eq 0 ]; then
    ...
    echo "PARTIAL: Only GoPro1 connected successfully"
    exit 1
elif [ $RESULT2 -eq 0 ]; then
    echo "PARTIAL: Only GoPro2 connected successfully"
    exit 1
else
