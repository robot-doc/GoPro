...
class GoProController:
    ...
    def download_latest_clip(self):
        ...
        try:
            ...
            print(f"[{self.name}] Starting download: {latest_name}")
            print(f"[{self.name}] Available space: {available_gb:.1f}GB")
        
        # Track download time
            download_start_time = time.time()
        
        # Use silent curl with no progress bar to avoid conflicts
            cmd = f"curl -s -m600 --connect-timeout 30 --speed-time 60 --speed-limit 1024 --interface {self.interface} -o '{final_dst}' '{camera_url}'"
        
        # Start download process
            download_process = subprocess.Popen(cmd, shell=True)
        
        # Monitor progress without interfering progress bars
            last_report_time = time.time()
            report_interval = 15  # Report every 15 seconds
        
            while download_process.poll() is None:  # While process is running
                ...
                if current_time - last_report_time >= report_interval:
                    try:
                        if os.path.exists(final_dst):
                            current_size_mb = os.path.getsize(final_dst) / (1024*1024)
                            elapsed_time = current_time - download_start_time
                            if elapsed_time > 0:
                                speed_mbps = (current_size_mb * 8) / elapsed_time
                                print(f"[{self.name}] Progress: {current_size_mb:.1f}MB downloaded in {elapsed_time:.0f}s ({speed_mbps:.1f} Mbps)")
                            else:
                                print(f"[{self.name}] Progress: {current_size_mb:.1f}MB downloaded")
                        else:
                            elapsed_time = current_time - download_start_time
                            print(f"[{self.name}] Progress: Download starting... ({elapsed_time:.0f}s elapsed)")
                        
                        last_report_time = current_time
                    except (OSError, FileNotFoundError):
                        elapsed_time = current_time - download_start_time
                        print(f"[{self.name}] Progress: Connecting... ({elapsed_time:.0f}s elapsed)")
                        last_report_time = current_time
        ...
        # Get final result
            ...
            if return_code == 0 and os.path.exists(final_dst):
                file_size_mb = os.path.getsize(final_dst) / (1024*1024)
                speed_mbps = (file_size_mb * 8) / download_time_sec if download_time_sec > 0 else 0
                print(f"[{self.name}]  Download completed: {file_size_mb:.1f}MB in {download_time_sec:.1f}s ({speed_mbps:.1f} Mbps)")
            ...
            # Verify file integrity (basic check - file size > 0 and reasonable)
                ...
                # File downloaded successfully, now delete from GoPro
                    if self.delete_file_from_gopro(latest_name):
                        print(f"[{self.name}]  Cleanup completed - removed from GoPro storage")
                    else:
                ...
                print(f"[{self.name}]  Download failed after {download_time_sec:.1f} seconds")
...
def record_and_fetch_all():
    """Record and fetch from both GoPros with sequential downloads, then queue video combination"""
    ...
    # SEQUENTIAL DOWNLOADS - Download from GoPros one at a time
    downloaded_files = {}
    download_results = {}
    print("[INFO] Starting sequential downloads to avoid network conflicts...")
    
    # Download from GoPro1 first
    gopro1_controller = controllers['gopro1']
    print(f"[INFO] Starting download from {gopro1_controller.name}...")
    try:
        result = gopro1_controller.download_latest_clip()
        download_results['gopro1'] = result
        if result:
            # Get the latest file from the controller's download directory
            files = [f for f in os.listdir(gopro1_controller.download_dir) 
                    if f.lower().endswith('.mp4')]
            if files:
                # Get the most recent file
                latest_file = max(files, key=lambda x: os.path.getctime(
                    os.path.join(gopro1_controller.download_dir, x)))
                downloaded_files['gopro1'] = os.path.join(
                    gopro1_controller.download_dir, latest_file)
                print(f"[INFO] {gopro1_controller.name} download completed successfully")
            else:
                print(f"[WARNING] {gopro1_controller.name} download reported success but no file found")
        else:
            print(f"[ERROR] {gopro1_controller.name} download failed")
    except Exception as e:
        print(f"[ERROR] Download failed for gopro1: {e}")
        download_results['gopro1'] = False
    
    # Small delay between downloads to avoid conflicts
    print("[INFO] Waiting 2 seconds before next download...")
    time.sleep(2)
    
    # Download from GoPro3 second
    gopro3_controller = controllers['gopro3']
    print(f"[INFO] Starting download from {gopro3_controller.name}...")
    try:
        result = gopro3_controller.download_latest_clip()
        download_results['gopro3'] = result
        if result:
            # Get the latest file from the controller's download directory
            files = [f for f in os.listdir(gopro3_controller.download_dir) 
                    if f.lower().endswith('.mp4')]
            if files:
                # Get the most recent file
                latest_file = max(files, key=lambda x: os.path.getctime(
                    os.path.join(gopro3_controller.download_dir, x)))
                downloaded_files['gopro3'] = os.path.join(
                    gopro3_controller.download_dir, latest_file)
                print(f"[INFO] {gopro3_controller.name} download completed successfully")
            else:
                print(f"[WARNING] {gopro3_controller.name} download reported success but no file found")
        else:
            print(f"[ERROR] {gopro3_controller.name} download failed")
    except Exception as e:
        print(f"[ERROR] Download failed for gopro3: {e}")
        download_results['gopro3'] = False
...
def main():
    ...
    print("[INFO] System ready - sequential downloads enabled, combinations run in background")
    ...
    try:
        while True:
            ...
            if not pcf_input.port[INPUT_PIN]:
                ...
                if connected_count == 2:
                    print(f"[INFO] Both GoPros connected - starting recording with sequential downloads")
                ...
                # Debounce
                start = time.time()
                while not pcf_input.port[INPUT_PIN] and (time.time() - start) < (DEBOUNCE_MS/1000):
